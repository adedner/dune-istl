...
namespace Dune
{
  ...
  /**
   * \brief Non-owning sparse matrix view over external block storage and pattern.
   *
   * MatrixView binds two external objects:
   * - a random-access iterator to block values,
   * - a sparsity pattern that defines row offsets and column indices.
   *
   * For row \f$i\f$, entries are stored contiguously, starting at
   * \f$\text{offset}(i)\f$ in the underlying block sequence, while
   * \c pattern()[i] provides the column indices for those entries.
   *
   * The view does not own memory. The caller must ensure that both the data
   * range and the sparsity pattern outlive the view. The pattern is treated as
   * read-only for the lifetime of the view.
   *
   * \tparam B Random-access iterator type for matrix blocks.
   * \tparam P Sparsity-pattern type with row ranges and offsets.
   */
  ...
  public:
    ...
    using size_type = pattern_type::size_type;
    ...
    using ConstColIterator = typename const_row_type::iterator;
  ...
  private:
    ...
    const size_type &baseIterator() const;
    size_type &baseIterator();
      ...
      : public Dune::IteratorFacade<ConstIterator, std::random_access_iterator_tag, const_row_type, const_row_type, Dune::ProxyArrowResult<const_row_type>>
  {
    ...
    /** \brief Return the current logical row index. */
    size_type index() const { return baseIterator().index(); }

    //! Compare const iterators with mutable iterators for equality and ordering.
    friend auto operator<=>(const ConstIterator &it1, const Iterator &it2)
    {
      return it1.baseIterator() <=> it2;
    }

    //! Compare const iterators with mutable iterators for equality and ordering.
    friend auto operator<=>(const Iterator &it1, const ConstIterator &it2)
    {
      return it1 <=> it2.baseIterator();
    }
  ...
  {
    std::ranges::for_each(std::span(data(), pattern().count()), [&k](block_type &b){
      b *= k;
    });
    return *this;
  }
  ...
  MatrixView<B, I> &MatrixView<B, I>::operator+=(const MatrixView<OtherB, OtherI> &b)
  {
    ...
    {
      std::ranges::for_each(Dune::range(pattern().count()), [&b, this](size_type i){
        data()[i] += b.data()[i];
      });
    }
    else if (pattern().size() == b.pattern().size() && pattern().range().size() == b.pattern().range().size())
    {
      std::ranges::for_each(Dune::range(pattern().size()), [&b, this](size_type i){
  ...
  {
    std::ranges::for_each(std::span(data(), pattern().count()), [&f](block_type &block){
      block = f;
    });
    return *this;
  }
  ...
  MatrixView<B, I> &MatrixView<B, I>::axpy(field_type alpha, const MatrixView<OtherB, OtherI> &b)
  {
    if (pattern_view_ == b.pattern_view_)
    {
      std::ranges::for_each(Dune::range(pattern().count()), [&b, alpha, this](size_type i){
        Impl::asMatrix(data()[i]).axpy(alpha, Impl::asMatrix(b.data()[i]));
      });
    }
    else if (pattern().size() == b.pattern().size() && pattern().range().size() == b.pattern().range().size())
    {
      std::ranges::for_each(Dune::range(pattern().size()), [&b, alpha, this](size_type i){
  ...
  void MatrixView<B, I>::umvImpl(const X &x, Y &y, F &&op, std::bool_constant<ClearRow>) const
  {
    ...
    for (size_type i = 0; i != N(); ++i) {
      ...
      std::ranges::for_each(pattern()[i], [i, &x, &y, &op, &nnz, this](size_type j) {
  ...
  template <std::random_access_iterator B, class I>
  const typename MatrixView<B, I>::size_type &MatrixView<B, I>::Iterator::baseIterator() const
  {
    return row_;
  }

  template <std::random_access_iterator B, class I>
  typename MatrixView<B, I>::size_type &MatrixView<B, I>::Iterator::baseIterator()
  {
    return row_;
  }
  ...
  template <std::random_access_iterator B, class I>
  MatrixView<B, I>::ConstIterator& MatrixView<B, I>::ConstIterator::operator=(const Iterator& it)
  {
    it_ = it.baseIterator();
    return *this;
  }
  ...
  {
    auto offset = baseIterator().pattern_view_->offset(index());
    return {baseIterator().data_view_, baseIterator().pattern_view_->operator[](index())};
  }

  template <std::random_access_iterator B, class I>
  const typename MatrixView<B, I>::Iterator &MatrixView<B, I>::ConstIterator::baseIterator() const
  {
    return it_;
  }

  template <std::random_access_iterator B, class I>
  typename MatrixView<B, I>::Iterator &MatrixView<B, I>::ConstIterator::baseIterator()
  {
    return it_;
  }
