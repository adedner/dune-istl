...
template<class M>
void setupSparsityPattern(M& A, int N)
{
 ...
 */
template<class M>
void setupLaplacian(M& A,
                    int N,
                    std::optional<typename Dune::FieldTraits<M>::field_type> reg = {} )
{
  using field_type = typename Dune::FieldTraits<M>::field_type;
  using block_type = typename M::block_type;

  setupSparsityPattern(A,N);

  block_type diagonal(static_cast<field_type>(0)), bone(static_cast<field_type>(0));
