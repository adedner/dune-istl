...
namespace Dune
{
  ...
  /** \brief Test a matrix against its explicitly supplied domain and range vectors.
   *
  ...
  void testMatrix(MatrixType& matrix, X& x, Y& y)
  {
    ...
    testVectorSpaceOperations(secondMatrix);
    testNorms(secondMatrix);
    testMatrixConstructibility<MatrixType>();
    testMatrixVectorProducts(secondMatrix, x, y);
  }
