...
public:
  ...
  using index_type = OrderedIndexRangeTraits<index_range_type>::index_type;
  //! The type used to count elements in the index set
  using size_type = OrderedIndexRangeTraits<index_range_type>::size_type;
...
private:
  ...
  public:
    ...
    constexpr reference operator*() const { return data_[offset_]; }

    //! Compare iterators for equality and ordering.
    friend auto operator<=>(const Iterator& it1, const Iterator& it2) noexcept
    {
      assert(it1.data_ == it2.data_ &&
             "Comparing iterators from different base arrays is not allowed");
      return it1.offset_ <=> it2.offset_;
    }

    //! Compare const iterators for equality and ordering.
    friend auto operator<=>(const Iterator& it1,
  ...
  public:
    ...
    //! Compare const iterators for equality and ordering.
    friend auto operator<=>(const ConstIterator& it1,
  ...
  private:
    ...
    constexpr const Iterator& baseIterator() const { return it_; }
    constexpr Iterator& baseIterator() { return it_; }
...
public:
  ...
  */
  iterator find(size_type i)
  {
    ...
    return (ofs == size() || index_set_[ofs] != i) ? end() : iterator(data_, index_set_, ofs);
  }
